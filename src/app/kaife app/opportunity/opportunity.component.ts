...
export class OpportunityComponent implements OnInit, OnDestroy {
    ...
    printOpp(op: any) {
        console.log('Pair '+op.A.paire);
        console.log('Opstatus '+op.opstatus);
        console.log('Orders count '+op.oporders.length);
        for(const order of op.oporders){
            console.log('Order key '+order.orkey);
            console.log('Orstatus '+order.orstatus);
        }
        console.log('OBJ '+JSON.stringify(op));
        console.log('----------------- ');
    }

    getOpportunity(op: any) {
        this.loadingState = ClrLoadingState.DEFAULT;
        op.oporders = op.oporders.sort((a: any, b: any) => {
            const ora = +a.orinitdate;
            const orb = +b.orinitdate;
            if (ora < orb) {
                return -1;
        ...
        for (let i = 0; i < this.ops.length; i++) {
            if (this.ops[i].opid === op.opid) {
                if(op.oporders.length == 0){
                    this.ops.splice(i, 1);                    
                    return;
                }
                /* for (let j = 0; j < this.ops[i].oporders.length; j++) {
                    if (this.ops[i].oporders[j].orkey == op.oporders[0].orkey) {
                        this.ops[i].oporders[j] = op.oporders[0]; 
                        console.log('Change order');
                        this.printOpp(op);
                        return;
                    }
                } 
                this.ops[i].oporders = op.oporders;
                this.printOpp(op);
                return;
            }
        }
